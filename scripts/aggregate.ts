...
// PINECONE_API_KEY and KIE_API_KEY — and deployment overrides like
// CLAUDE_OS_HOME — are available without a manual export. Loaded FIRST so the
// path constants below can honor overrides from the file.
...
// Operator-home resolution (see vault: 07 System/Mission Control/). Upstream
// assumed the aggregator runs in the operator's own account, so ~ held all the
// signal (.claude, .claude.json, .codex, .hermes, Obsidian folders, ...). On a
// VPS the aggregator often runs as a service user whose home is empty, with the
// real data elsewhere (another user, a container bind-mount). CLAUDE_OS_HOME
// redirects the entire scan root; ~25 detection sites inherit it.
const HOME = process.env.CLAUDE_OS_HOME || homedir();
// Claude data dir: explicit override → <home>/.claude → a containerized Claude
// Code's bind-mounted home (conventional /docker/<stack>/claude-home layout).
const CLAUDE_DIR = (() => {
  if (process.env.CLAUDE_OS_CLAUDE_DIR) return process.env.CLAUDE_OS_CLAUDE_DIR;
  const p = join(HOME, ".claude");
  if (existsSync(p)) return p;
  try {
    for (const d of readdirSync("/docker")) {
      const c = join("/docker", d, "claude-home");
      if (existsSync(c)) return c;
    }
  } catch {
    /* no /docker layout on this machine */
  }
  return p;
})();
const PROJECTS_DIR = join(CLAUDE_DIR, "projects");
// ~/.claude.json (MCP servers, onboarding state) lives NEXT TO ~/.claude, so a
// container that only bind-mounts the .claude dir won't expose it at <home>.
// Check the operator home first, then inside / alongside CLAUDE_DIR.
const CLAUDE_JSON_PATH = (() => {
  if (process.env.CLAUDE_OS_CLAUDE_JSON) return process.env.CLAUDE_OS_CLAUDE_JSON;
  const candidates = [
...
// Agent-fulfilled Dreams (see vault: 91 Competition/Hermes Write-Control
// Scope.md): optional drop-folder contract letting an always-on agent (e.g. a
// containerized Hermes) fulfill the nightly Dream — no CLI and no API key on
// this machine. The aggregator exports live-data.json into the folder (the
// agent's inbox) and ingests finished dreams from <dir>/dreams (its outbox).
// Off unless CLAUDE_OS_AGENT_DREAM_DIR is set; missing dirs are skipped quietly.
const AGENT_DREAM_DIR = process.env.CLAUDE_OS_AGENT_DREAM_DIR || "";
...
async function detectClaudeAuth(usage?: {
  ...
  // Config-first override for headless/VPS boxes, where the usage heuristic is
  // blind (sessions live in containers or on other machines) and would guess
  // the cheapest tier at low confidence. CLAUDE_OS_CLAUDE_PLAN: pro|max_5x|max_20x.
  const declared = (process.env.CLAUDE_OS_CLAUDE_PLAN || "").toLowerCase();
  const declaredPlan =
    declared === "max_20x"
      ? "Claude Max 20x"
      : declared === "max_5x"
  ...
  if (declaredPlan) {
    return {
      authMode,
      credCount,
      planGuess: declaredPlan,
      planConfidence: "high" as const,
      evidence: [`plan declared via CLAUDE_OS_CLAUDE_PLAN: ${declaredPlan}`],
    };
  }
...
type MemSource = "obsidian" | "claude" | "pinecone" | "qdrant";
...
/**
 * Cheap reachability probe, independent of auth outcome. Distinguishes
 * "server is up but rejects our key/missing key" (still worth surfacing as
 * configured-and-reachable) from "nothing is listening / network error".
 */
async function pingQdrant(): Promise<{ reachable: boolean; authOk: boolean }> {
  ...
  try {
    ...
    // 401/403 = server reachable, credentials wrong/missing.
    if (res.status === 401 || res.status === 403) return { reachable: true, authOk: false };
    return { reachable: true, authOk: res.ok };
  } catch {
    return { reachable: false, authOk: false };
  }
}

// Qdrant — self-hosted vector DB. Not in the upstream Claude OS scanner (README
// lists it as an explicit known gap); this mirrors fetchPineconeIndexes() against
// Qdrant's REST API. QDRANT_URL defaults to the standard local port; QDRANT_API_KEY
// is required whenever the instance enforces auth (ours does).
async function fetchQdrantCollections(): Promise<QdrantCollection[]> {
  const url = qdrantBaseUrl();
  const key = process.env.QDRANT_API_KEY;
  try {
    ...
    if (!list.ok) return [];
    const j: any = await list.json();
    const names: string[] = (j.result?.collections || []).map((c: any) => c.name);

    const settled = await Promise.allSettled(
      names.map(async (name) => {
        const info = await fetch(`${url}/collections/${encodeURIComponent(name)}`, {
          headers,
          signal: AbortSignal.timeout(5000),
        });
        if (!info.ok) throw new Error(`collection ${info.status}`);
        const j2: any = await info.json();
        const r = j2.result || {};
        return {
          name,
          vectorCount: r.vectors_count ?? r.points_count ?? 0,
    ...
    return settled
      .filter((r): r is PromiseFulfilledResult<QdrantCollection> => r.status === "fulfilled")
      .map((r) => r.value);
  } catch {
    return [];
  }
...
async function parseMemory() {
  ...
  // ── Qdrant vector-store nodes — self-hosted twin of the Pinecone block
  // above, so VPS/local-first stacks light up the constellation the same way
  // Pinecone cloud does (see vault: 07 System/Mission Control/). ──
  console.log("[aggregate] polling Qdrant ...");
  const qdrantCols = await fetchQdrantCollections();
  const qdrantDisplay = qdrantCols
    .slice()
    .sort((a, b) => a.name.localeCompare(b.name))
    .map((col, index) => ({
      col,
      label: SHOW_INDEX_NAMES ? (sanitize(col.name) ?? col.name) : `qdrant-store-${index + 1}`,
    }));
  for (const { col, label } of qdrantDisplay) {
    const tone = tonalForIndex(label);
    const id = `qd-${slugify(label)}`;
    nodes.push({
      id,
      name: label,
      kind: "vector_store",
      source: "qdrant",
      indexName: label,
      vectorCount: col.pointsCount,
      namespaces: [],
      val: 28,
      color: tone,
      meta: `${col.pointsCount.toLocaleString()} points · ${col.status}`,
    });
    links.push({ source: "hub", target: id, kind: "vector" });
    sourceList.push({
      kind: "qdrant",
  ...
  const totalVectors =
    pineconeIndexes.reduce((a, i) => a + i.totalVectorCount, 0) +
    qdrantCols.reduce((a, c) => a + (c.pointsCount || 0), 0);
  ...
  return {
    ...
    stats: {
      ...
      qdrantCollections: qdrantCols.length,
    },
    qdrant: qdrantDisplay.map(({ col, label }) => ({
      name: label,
...
async function detectApps(): Promise<AppDetections> {
  ...
  // Container fallback (config-first — see vault: 07 System/Mission Control/):
  // on VPS deployments Claude Code typically runs as a Docker container with a
  // bind-mounted home dir. Honor an explicit override first, then scan the
  // conventional /docker/<stack>/claude-home layout so any stack name works.
  const claudeCodeContainerHome = (() => {
    if (process.env.CLAUDE_OS_CLAUDE_CODE_HOME) return process.env.CLAUDE_OS_CLAUDE_CODE_HOME;
    try {
      for (const d of readdirSync("/docker")) {
        const p = join("/docker", d, "claude-home");
        if (existsSync(p)) return p;
      }
    } catch {
      /* no /docker layout on this machine */
  ...
  const claudeCode: AppDetection = claudeCli
    ...
    : claudeCodeContainerHome
      ? {
          detected: true,
          path: sanitize(claudeCodeContainerHome),
  ...
  // Container fallback: the hermes-agent container publishes its dashboard on
  // host loopback :9119. Any HTTP answer (even a redirect) means Hermes runs here.
  const hermesAgentUrl = process.env.CLAUDE_OS_HERMES_URL || "http://127.0.0.1:9119";
  let hermesContainerUp = false;
  if (!hermesInstalled) {
    try {
      const res = await fetch(`${hermesAgentUrl}/`, {
        redirect: "manual",
        signal: AbortSignal.timeout(1500),
      });
      hermesContainerUp = res.status >= 200 && res.status < 500;
    } catch {
  ...
  const hermes: AppDetection = hermesInstalled
    ...
    : hermesContainerUp
      ? { detected: true, path: hermesAgentUrl, name: "Hermes Agent (Docker)" }
      : { detected: false };
...
async function detectMemoryStores(
  memory: {
    sources?: any[];
  ...
  const qdrantHasKey = Boolean(process.env.QDRANT_API_KEY);
  const { reachable: qdrantReachable, authOk: qdrantAuthOk } = await pingQdrant();
  const qdrantCollectionCount = qdrantCollections.length;
  const qdrantTotalVectors = qdrantCollections.reduce((a, c) => a + (c.vectorCount || 0), 0);
  const qdrant = {
    detected: qdrantAuthOk || qdrantHasKey,
    hasKey: qdrantHasKey,
    reachable: qdrantReachable,
    ...(qdrantReachable ? { url: qdrantBaseUrl() } : {}),
    ...(qdrantCollectionCount > 0
      ? { collections: qdrantCollectionCount, totalVectors: qdrantTotalVectors }
      : {}),
  };
  ...
  return { pinecone, qdrant, obsidian, notion, logseq };
}
...
function computeEnvKeysNeeded(envKeysPresent: string[]): EnvKeyNeeded[] {
  const candidates: Array<{
    ...
    // Self-hosted / VPS stack keys (see vault: 07 System/Mission Control/) —
    // everything the integrations panel understands should be discoverable
    // here, so out-of-box users can see what a key would unlock.
    {
      name: "QDRANT_API_KEY",
      reason:
        "Secures your self-hosted Qdrant instance; the dashboard reads its collections into the memory constellation.",
    ...
    },
    {
      ...
      reason:
        "Lets the dashboard read Hermes sessions and memory through the Hermes dashboard API (pair with HERMES_DASHBOARD_USERNAME).",
      optional: true,
    },
    {
      name: "DISCORD_BOT_TOKEN",
      reason: "Lights up the Discord integration tile for Discord-side automations.",
      url: "https://discord.com/developers/applications",
      optional: true,
    },
    {
      name: "MAKE_API_KEY",
      reason: "Connects Make.com scenarios to the integrations panel.",
      url: "https://www.make.com/en/api-documentation",
      optional: true,
    },
    {
      name: "N8N_API_URL",
      reason: "Points the dashboard at your n8n instance (pair with N8N_API_KEY).",
      optional: true,
    },
    {
      name: "BROWSERBASE_API_KEY",
      reason: "Enables the Browserbase tile for cloud browser automations.",
...
/**
 * Docker/remote fallback for gatherHermes(). Same-machine Claude OS assumes
 * Hermes lives at ~/.hermes on the machine running the aggregator; when Hermes
 * runs in a separate Docker container (our topology) that path never exists,
 * so the local branch below always returned null — "not installed" — even
 * though Hermes is alive and reachable. This queries the Hermes Dashboard API
 * instead of the filesystem: status is public (no creds needed, gives version
 * + reachability); skills/sessions/model additionally require
 * HERMES_DASHBOARD_USERNAME + HERMES_DASHBOARD_PASSWORD (the same basic-auth
 * cookie login the Workspace fix uses) and degrade gracefully if unset.
 * Memory file contents (SOUL.md/USER.md/MEMORY.md) are intentionally left
 * blank here — the dashboard has no endpoint that returns their raw text, and
 * we'd rather show nothing than fabricate it.
 */
async function gatherHermesFromDashboard(): Promise<HermesSnapshot | null> {
  const dashboardUrl = (process.env.HERMES_DASHBOARD_URL || "").replace(/\/+$/, "");
  if (!dashboardUrl) return null;

  let status: any = null;
  try {
    const res = await fetch(`${dashboardUrl}/api/status`, { signal: AbortSignal.timeout(5000) });
  ...
  if (username && password) {
    try {
      const login = await fetch(`${dashboardUrl}/auth/password-login`, {
        method: "POST",
      ...
      /* proceed unauthenticated — skills/sessions/model stay at defaults */
    }
  }
  ...
  if (cookie) {
    try {
      const res = await fetch(`${dashboardUrl}/api/skills`, {
        headers: { Cookie: cookie },
        signal: AbortSignal.timeout(5000),
      });
      if (res.ok) {
        const j: any = await res.json();
        skillCount = (Array.isArray(j) ? j : (j.skills ?? j.data ?? [])).length;
      }
    } catch {
      /* leave at 0 */
    }
    ...
    try {
      const res = await fetch(`${dashboardUrl}/api/sessions?limit=8&offset=0`, {
        headers: { Cookie: cookie },
        signal: AbortSignal.timeout(5000),
      });
      if (res.ok) {
        const j: any = await res.json();
        sessionCount = j.total ?? (j.sessions ?? j.data ?? []).length;
        for (const s of j.sessions ?? j.data ?? []) {
          const lastActive = s.last_active ? s.last_active * 1000 : null;
          if (lastActive && (!lastActiveMs || lastActive > lastActiveMs)) lastActiveMs = lastActive;
          recentSessions.push({
            id: String(s.id ?? ""),
            firstUserMessage: sanitize(s.preview ?? null) ?? null,
            model: s.model ?? null,
            platform: s.source ?? null,
            lastUpdatedMs: lastActive,
            lastUpdatedAgo: lastActive ? humanAgo(Date.now() - lastActive) : "unknown",
    ...
    try {
      ...
      if (res.ok) {
        const j: any = await res.json();
        defaultModel = typeof j.model === "string" ? j.model : (j.model?.default ?? null);
      }
    } catch {
      /* leave null */
    }
  ...
  return {
    installed: true,
    hermesHome: `${dashboardUrl} (Docker — dashboard API${cookie ? "" : ", unauthenticated"})`,
    ...
    recentSessions,
    lastActiveMs,
    lastActiveAgo: lastActiveMs ? humanAgo(Date.now() - lastActiveMs) : "unknown",
  };
}

async function gatherHermes(): Promise<HermesSnapshot | null> {
  const hermesHome = process.env.HERMES_HOME || join(HOME, ".hermes");
  if (!existsSync(hermesHome)) return gatherHermesFromDashboard();
...
async function loadLatestDream() {
  ...
  // Local engine output + (optionally) the agent-fulfilled outbox. The newest
  // dated file across ALL sources wins, so a nightly agent dream and a manual
  // "Generate now" run coexist without special cases.
  const dreamDirs = [join(HOME, ".claude-os", "dreams")];
  if (AGENT_DREAM_DIR) dreamDirs.push(join(AGENT_DREAM_DIR, "dreams"));
  let best: { dir: string; file: string } | null = null;
  for (const dir of dreamDirs) {
    ...
    for (const f of entries) {
      if (!/^dream-\d{4}-\d{2}-\d{2}\.json$/.test(f)) continue;
      if (!best || f > best.file) best = { dir, file: f };
    }
  }
  if (!best) return null;
  try {
    const raw = readFileSync(join(best.dir, best.file), "utf-8");
    const parsed = JSON.parse(raw);
    return parsed;
  } catch (err) {
    console.warn(`[aggregate] failed to parse latest dream JSON (${best.file}):`, err);
    return null;
  }
}
...
async function main() {
  ...
  const qdrantForDetect = await fetchQdrantCollections();
  const memoryStores = await detectMemoryStores(memory as any, pineconeForDetect, qdrantForDetect);
  ...
  const data = {
    ...
    subscriptions: {
      ...
      chatgpt: chatgpt.present
        ...
        : (() => {
            // Config-first declaration for stacks where the ChatGPT plan is
            // consumed by an agent (e.g. a containerized Hermes on openai-codex)
            // rather than a local ~/.codex install the scanner could see.
            // CLAUDE_OS_CHATGPT_PLAN: plus|team|pro.
            const d = (process.env.CLAUDE_OS_CHATGPT_PLAN || "").toLowerCase();
            if (!d) return { present: false, plan: null, monthlyPrice: 0 };
            const plan =
              d === "pro" ? "ChatGPT Pro" : d === "team" ? "ChatGPT Team" : "ChatGPT Plus";
            const price = d === "pro" ? 200 : d === "team" ? 25 : 20;
            return {
              present: true,
              plan,
              monthlyPrice: price,
              confidence: "high" as const,
              evidence: [`plan declared via CLAUDE_OS_CHATGPT_PLAN: ${plan}`],
            };
          })(),
    ...
    integrations: (() => {
      ...
      // Qdrant — self-hosted vector DB. Status-driven (live reachability
      // probe), so VPS/local-first stacks get a real tile without a cloud key.
      if (memoryStores.qdrant?.reachable) {
        out.push({
          name: "Qdrant",
          slug: "qdrant",
          connected: true,
          tagline: `${(memory as any).qdrant?.length ?? 0} collections · self-hosted`,
          color: "DC244C",
        });
      }
      if (openclaw) {
      ...
      const optional: Array<{
        ...
        // Added for Adil's stack (2026-07-07): Discord/Make had zero hooks
        // anywhere in the app; n8n/Browserbase were only detectable via Mac-side
        // ~/.claude.json (MCP servers) / Claude.ai connectors, which this VPS
        // process can't see. These give them the same lightweight env-var path
        // as everything else above — set the var whenever configured, no code
        // changes needed at that point.
        {
          env: "DISCORD_BOT_TOKEN",
          entry: { name: "Discord", slug: "discord", tagline: "bot token set", color: "5865F2" },
        },
        {
          // Kie powers Dream-card image generation behind the scenes; without
          // a tile, a freshly pasted key looks like it did nothing.
          env: "KIE_API_KEY",
          entry: { name: "Kie.ai", slug: "kie", tagline: "image generation ready", color: "8B5CF6" },
        },
        {
          env: "MAKE_API_KEY",
          entry: { name: "Make", slug: "make", tagline: "API key set", color: "6D00CC" },
        },
        {
          env: "MAKE_WEBHOOK_URL",
          entry: { name: "Make", slug: "make", tagline: "webhook set", color: "6D00CC" },
        },
        {
          env: "N8N_API_URL",
          entry: { name: "n8n", slug: "n8n", tagline: "instance URL set", color: "FF6D5A" },
        },
        {
          env: "N8N_API_KEY",
          entry: { name: "n8n", slug: "n8n", tagline: "API key set", color: "FF6D5A" },
        },
        {
          env: "BROWSERBASE_API_KEY",
          entry: { name: "Browserbase", slug: "browserbase", tagline: "API key set", color: "FF6B35" },
        },
      ];
  ...
  // Agent-fulfilled Dreams: refresh the agent's inbox copy so its nightly job
  // always reads data at most one aggregator tick old.
  if (AGENT_DREAM_DIR && existsSync(AGENT_DREAM_DIR)) {
    try {
      await Bun.write(join(AGENT_DREAM_DIR, "live-data.json"), JSON.stringify(emitted, null, 2));
    } catch (err: any) {
      console.warn(`[aggregate] agent-dream inbox export skipped: ${err?.message}`);
    }
  }
