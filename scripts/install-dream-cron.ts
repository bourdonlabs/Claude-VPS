 ...
 *
 * Linux / other:
 *   - Manages the invoking user's crontab non-interactively (`crontab -l` /
 *     `crontab -`), keeping our entry inside `# claude-os-dream BEGIN/END`
 *     sentinel markers so re-installs and uninstalls are idempotent. Falls
 *     back to printing the manual snippet if `crontab` isn't installed.
 *
 * Usage:
...
// Sentinel markers so install/uninstall can find and replace exactly our
// block in the user's crontab without touching their other entries.
const CRON_MARKER_BEGIN = "# claude-os-dream BEGIN";
const CRON_MARKER_END = "# claude-os-dream END";

// Build the daily cron entry. Mirrors the macOS plist semantics exactly:
// refresh live-data.json first, then fire /dream (`;` not `&&` so a flaky
// aggregator run doesn't block the review), all appended to the log. Wrapped
// in `/bin/sh -lc '...'` so the user's login PATH is available (cron's own
// env is minimal), with every embedded path single-quoted.
function buildLinuxCronLine(hour: number, minute: number): string {
  const cdPart = `cd ${shellSingleQuote(REPO_ROOT)}`;
  const bunPart = `${shellSingleQuote(BUN_BIN)} run scripts/aggregate.ts`;
  const dreamPart = buildDreamCommand();
  const inner = `${cdPart} && ${bunPart} ; ${dreamPart} >> ${shellSingleQuote(LOG_PATH)} 2>&1`;
  return `${minute} ${hour} * * * /bin/sh -lc ${shellSingleQuote(inner)}`;
}

// Read the invoking user's crontab. A non-zero exit (typically "no crontab
// for <user>") is treated as an empty crontab, not an error.
function readCrontab(): string {
  const r = spawnSync("crontab", ["-l"], { stdio: "pipe", encoding: "utf-8" });
  if (r.status !== 0) return "";
  return r.stdout ?? "";
}

// Write the given content back as the user's crontab via `crontab -`.
function writeCrontab(content: string): boolean {
  ...
  if (r.status !== 0) {
    console.error("[install-dream-cron] `crontab -` failed.");
    if (r.stderr) console.error(r.stderr.trim());
    return false;
  }
  return true;
...
// Remove any existing claude-os-dream block (markers included) so re-installs
// are idempotent and uninstall leaves the rest of the crontab untouched.
function stripDreamBlock(crontab: string): string {
  ...
  // Drop trailing blank lines so repeated install/uninstall cycles don't
  // accrete whitespace at the end of the crontab.
...
function hasCrontabBinary(): boolean {
  try {
    const out = spawnSync(whichCommand(), ["crontab"], { encoding: "utf-8" }).stdout ?? "";
    return out.split(/\r?\n/).some((l) => l.trim() && existsSync(l.trim()));
  } catch {
    return false;
  }
}

function installLinux(hour: number, minute: number, rawTime: string) {
  ensureDirs();
  if (!HERMES_BIN) {
    warnIfClaudeBinUnresolved();
    warnIfHeadlessAuthMissing();
  ...
  const cronLine = buildLinuxCronLine(hour, minute);

  if (!hasCrontabBinary()) {
    console.error("[install-dream-cron] `crontab` not found on this machine.");
    console.error("  Install cron first (e.g. `apt install cron`), then re-run:");
    console.error("  bun run install-dream");
    console.log("");
    printLinuxFallback(cronLine, rawTime);
    process.exit(1);
  }

  const existing = stripDreamBlock(readCrontab());
  const block = [CRON_MARKER_BEGIN, cronLine, CRON_MARKER_END].join("\n");
  const next = (existing ? `${existing}\n\n` : "") + block + "\n";
  if (!writeCrontab(next)) {
    console.error("");
    console.error("[install-dream-cron] could not update your crontab.");
    console.error("  Add this line manually (run `crontab -e`):");
    console.error("");
    console.error(`    ${cronLine}`);
    process.exit(1);
  }
  console.log(`[install-dream-cron] wrote crontab entry (${CRON_MARKER_BEGIN} … ${CRON_MARKER_END}).`);
  ...
  console.log(`fires /dream so prescriptions are based on up-to-date metrics.`);
  console.log(`Logs: ${LOG_PATH}`);
  console.log(`Crontab entry: crontab -l  (between the claude-os-dream markers)`);
  console.log("");
  console.log("To trigger an on-demand run right now:");
  console.log(`  cd ${REPO_ROOT} && bun run scripts/aggregate.ts && bun run scripts/run-dream.ts`);
  console.log("");
...
function uninstallLinux() {
  if (!hasCrontabBinary()) {
    console.log("[install-dream-cron] `crontab` not found — nothing to uninstall.");
    return;
  }
  const current = readCrontab();
  if (!current.includes(CRON_MARKER_BEGIN)) {
    console.log("[install-dream-cron] no claude-os-dream crontab entry found — nothing to do.");
    return;
  }
  const stripped = stripDreamBlock(current);
  if (!writeCrontab(stripped ? `${stripped}\n` : "")) {
    console.error("[install-dream-cron] could not update your crontab.");
    console.error("  Remove the claude-os-dream block manually (run `crontab -e`).");
    process.exit(1);
  }
  console.log("[install-dream-cron] removed claude-os-dream crontab entry.");
  console.log(
    `Dream cron uninstalled. Log file kept at ${LOG_PATH} (delete manually if you want).`,
  );
}

// Printed only when `crontab` itself is missing — gives the user the exact
// line to install by hand once cron is available.
function printLinuxFallback(cronLine: string, rawTime: string) {
  console.log(`To install the Dream cron manually on Linux at ${rawTime} daily:`);
  console.log("");
  console.log("  1. Make sure ~/.claude-os/dreams exists:");
  console.log(`       mkdir -p ${shellSingleQuote(DREAM_DIR)}`);
  console.log("");
  console.log("  2. Add this line to your crontab (run `crontab -e`):");
  console.log("");
  console.log(`       ${cronLine}`);
  console.log("");
  console.log("  3. It refreshes live-data.json (scripts/aggregate.ts) before each Dream run.");
...
function main() {
  ...
  if (process.platform !== "darwin") {
    ...
    installLinux(args.hour, args.minute, args.rawTime);
    return;
  }
