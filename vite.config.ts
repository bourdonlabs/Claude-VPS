...
// Deployment config from app/.env.local (see vault: 07 System/Mission Control/).
// vite.config.ts runs before Vite's own env loading, so parse the file once here;
// process.env always wins so systemd/shell overrides behave normally.
const DOTENV: Record<string, string> = (() => {
  const out: Record<string, string> = {};
  try {
    for (const line of readFileSync(join(resolve(__dirname), ".env.local"), "utf-8").split("\n")) {
...
// Remote/VPS deployment: when Claude OS runs on a server behind a reverse proxy
// (Tailscale serve, nginx, Caddy, Cloudflare Tunnel — anything that proxies from
// this machine's loopback), the hostname(s) that proxy forwards become
// first-class trusted origins, exactly equivalent to the localhost origin the
// app was originally written for. Configure via CLAUDE_OS_TRUSTED_HOSTS
// (comma-separated, exact hostnames) in the environment or app/.env.local.
// Empty = localhost-only (secure default). Also reused by server.allowedHosts.
const TRUSTED_HOSTS: string[] = (process.env.CLAUDE_OS_TRUSTED_HOSTS ?? DOTENV.CLAUDE_OS_TRUSTED_HOSTS ?? "")
  .split(",")
  .map((h) => h.trim().toLowerCase())
  .filter(Boolean);

// Hermes home resolution: where the Hermes Agent's state lives (config.yaml,
// skills/, sessions/, memories/, missions.json, pantheon/ ...). Mac default is
// ~/.hermes; on a VPS set HERMES_HOME (env or app/.env.local) to wherever
// Hermes's data directory is mounted. Every /__hermes_* endpoint resolves
// through this ONE constant — Mission Control is the operator's CONTROL panel,
// so this is intentionally the same read-write access the Mac layout has.
const HERMES_HOME_DIR =
  process.env.HERMES_HOME ?? DOTENV.HERMES_HOME ?? join(homedir(), ".hermes");
...
function isLoopback(req: { socket?: { remoteAddress?: string | null }; headers?: Record<string, any> }): boolean {
  ...
  // ever sends a loopback hostname — or one of this deployment's configured
  // TRUSTED_HOSTS via its reverse proxy — for our real origin, so reject anything else.
...
export default defineConfig({
  ...
  vite: {
    ...
    plugins: [
      {
        ...
        configureServer(server) {
          // Reverse-proxy compatibility: some upstream proxies normalize
          // /mission-control/ to /mission-control. TanStack Start expects the
          // configured basepath with a trailing slash, so redirect explicitly.
          server.middlewares.use((req, res, next) => {
            if (req.url === "/mission-control") {
              res.statusCode = 302;
              res.setHeader("Location", "/mission-control/");
              res.end();
              return;
            }
            next();
          ...
          // Sub-path fix (see vault: 07 System/Mission Control/): this app is
          // served under base "/mission-control/", but its ~40 /__* API endpoints
          // are registered at root (/__live-data, /__hermes_status, ...). The
          // reverse proxy only forwards /mission-control/* here, so normalize
          // /mission-control/__X -> /__X BEFORE those handlers run. Pairs with the
          // browser fetch shim in src/router.tsx. Do NOT remove one without the other.
          // EXCEPT Vite's own internals (__vite_ping HMR heartbeat, editor/devtools
          // helpers), which legitimately live at <base>/__* and must NOT be stripped.
          const VITE_INTERNAL = /^\/mission-control\/__(vite|open-in-editor|devtools|inspect)/;
          server.middlewares.use((req, _res, next) => {
            if (
              req.url &&
              req.url.startsWith("/mission-control/__") &&
              !VITE_INTERNAL.test(req.url)
            ) {
              req.url = req.url.slice("/mission-control".length); // -> /__X
            }
            next();
          });

          // .env.local is THE canonical config file — the wizard writes it and
          // every layer reads it. Vite's own in-place restart on env changes is
          // unreliable under bun (observed wedging: process alive, listener
          // gone). When supervised by systemd (Restart=always), replace that
          // fragile path with a deterministic clean exit — systemd respawns a
          // fresh server in ~3s. Under a bare `bun run dev` (no supervisor),
          // leave vite's default behavior alone.
          if (process.env.INVOCATION_ID) {
            try {
              let exiting = false;
              fsWatch(join(resolve(__dirname), ".env.local"), () => {
                if (exiting) return;
                exiting = true;
                console.log("[claude-os] .env.local changed — clean restart via systemd");
                setTimeout(() => process.exit(0), 150);
              });
            } catch {
              /* no .env.local on this machine yet — nothing to watch */
            }
          }
          ...
          server.middlewares.use("/__hermes_moa_save", (req, res, next) => {
            ...
            req.on("end", () => {
              try {
                ...
                const configPath = join(HERMES_HOME_DIR,"config.yaml");
          ...
          // POST /__save_env_keys — persist wizard-collected API keys into
          // .env.local (the file scripts/aggregate.ts loads). MERGE-only:
          // existing keys are never overwritten (reported as "skipped");
          // new keys are appended. Timestamp-backs-up the file first.
          // Loopback + per-run token gated. Never echoes values.
          server.middlewares.use("/__save_env_keys", (req, res, next) => {
            if (req.method !== "POST") return next();
            res.setHeader("Content-Type", "application/json");
            if (!isLoopback(req)) {
              res.statusCode = 403;
            ...
            req.on("end", () => {
              try {
                ...
                if (
                  ...
                  res.statusCode = 400;
                  res.end(JSON.stringify({ error: "keys must be an object" }));
                  return;
                }
                const KEY_RE = /^[A-Z][A-Z0-9_]{2,40}$/;
                const envPath = join(resolve(__dirname), ".env.local");
                const fileExists = existsSync(envPath);
                const existing = fileExists ? readFileSync(envPath, "utf-8") : "";
                // Names already defined in the file — existing user secrets win.
                ...
                for (const [rawName, rawValue] of Object.entries(keys)) {
                  ...
                  if (!value) continue; // skip empty values silently
                ...
                if (added.length > 0) {
                  // backup before any write (same pattern as __hermes_moa_save)
                  if (fileExists) {
                    const stamp = new Date().toISOString().replace(/[:.]/g, "-");
                    writeFileSync(`${envPath}.bak-${stamp}`, existing, {
                      mode: 0o600,
                    });
                  }
                  const sep2 =
                  ...
                  const block =
                    `${sep2}# added by Claude OS setup wizard ${new Date().toISOString()}\n` +
                    newLines.join("\n") +
                    "\n";
                  // mode only applies on creation; existing mode is preserved.
                  writeFileSync(envPath, existing + block, { mode: 0o600 });
                }
          ...
          server.middlewares.use("/__hermes_status", (req, res, next) => {
            ...
            const home = homedir();
            const hermesDir = HERMES_HOME_DIR;
          ...
          server.middlewares.use("/__hermes_image_upload", (req, res, next) => {
            ...
            req.on("end", () => {
              ...
              const cacheDir = join(HERMES_HOME_DIR,"image_cache");
              try {
          ...
          server.middlewares.use("/__hermes_chat", async (req, res, next) => {
            ...
            const home = homedir();
            const hermesRoot = join(HERMES_HOME_DIR,"hermes-agent");
          ...
          const MISSIONS_FILE = join(HERMES_HOME_DIR,"missions.json");
          ...
          server.middlewares.use("/__hermes_skills", (req, res, next) => {
            ...
            const skillsDir = join(HERMES_HOME_DIR,"skills");
            const out: Array<{
              id: string;
              description: string;
              subskills: string[];
          ...
          server.middlewares.use("/__hermes_connections", (req, res, next) => {
            ...
            try {
              const authPath = join(HERMES_HOME_DIR,"auth.json");
            ...
            try {
              const envPath = join(HERMES_HOME_DIR,".env");
          ...
          const pantheonDir = join(HERMES_HOME_DIR,"pantheon", "personas");
          const pantheonAssetsDir = join(HERMES_HOME_DIR,"pantheon", "assets");
          ...
          server.middlewares.use("/__hermes_models", (req, res, next) => {
            ...
            try {
              const cfgPath = join(HERMES_HOME_DIR,"config.yaml");
            ...
            try {
              const cfgPath2 = join(HERMES_HOME_DIR,"config.yaml");
            ...
            try {
              const cfg = yaml.load(
                readFileSync(join(HERMES_HOME_DIR,"config.yaml"), "utf-8"),
              ) as any;
          ...
          server.middlewares.use("/__hermes_pantheon_sync", (req, res, next) => {
            ...
            if (!mirrorRoot) {
              try {
                const markerPath = join(HERMES_HOME_DIR,".mirror_path");
          ...
          server.middlewares.use("/__hermes_memory", (req, res, next) => {
            ...
            const hermesHome = HERMES_HOME_DIR;
          ...
          server.middlewares.use("/__hermes_sessions", (req, res, next) => {
            ...
            const sessionsDir = join(HERMES_HOME_DIR,"sessions");
            const out: Array<{
              id: string;
              model: string | null;
          ...
          server.middlewares.use("/__set_dream_engine", (req, res, next) => {
            ...
            req.on("end", () => {
              ...
              const valid = ["hermes", "claude", "openrouter", "codex", "hermes-agent"];
          ...
          server.middlewares.use("/__dream_engines", (req, res, next) => {
            ...
            const hermesSkill = existsSync(join(HERMES_HOME_DIR,"skills", "dream", "SKILL.md"));
            const hermesConfig = existsSync(join(HERMES_HOME_DIR,"config.yaml"));
            // Container/remote deployment: no runnable CLI on this machine, but
            // a real Hermes home (config.yaml) is mounted — Hermes exists, the
            // dashboard just can't exec it directly. Report installed + honest
            // needsAction instead of pretending it isn't there.
            const hermesContainer = !hermesBin && hermesConfig;
            engines.push({
              id: "hermes",
              name: "Hermes",
              description: hermesContainer
                ? "Hermes detected (runs in its own container/remote) — the dashboard can't execute it directly"
                : "Routes via your Hermes provider (gpt-5.5 / OpenAI by default) — sends your activity to that provider",
              installed: !!hermesBin || hermesContainer,
              ready: !!hermesBin && hermesSkill && hermesConfig,
              needsAction: hermesContainer
                ? "Runs in a container — pick OpenRouter for Dream, or install the Hermes CLI on this machine"
                : !hermesBin
            ...
            // Hermes (agent-fulfilled) — the VPS-native engine. An always-on
            // agent (e.g. a containerized Hermes) fulfills the nightly Dream
            // through a drop-folder in its home: the aggregator exports
            // live-data.json into it (inbox), the agent's own scheduled job
            // writes the finished dream to <dir>/dreams (outbox), and the
            // aggregator ingests it. No CLI and no API key on this machine —
            // it runs on whatever subscription the agent already uses.
            const agentDreamDir =
              process.env.CLAUDE_OS_AGENT_DREAM_DIR ?? DOTENV.CLAUDE_OS_AGENT_DREAM_DIR ?? "";
            const agentDreamReady = !!agentDreamDir && existsSync(agentDreamDir);
            engines.push({
              id: "hermes-agent",
              name: "Hermes (agent-fulfilled)",
              description:
                "Your always-on agent writes the nightly Dream via a drop-folder — no CLI or API key on this machine. Schedule the agent's job for the early hours of your morning; results appear automatically.",
              installed: hermesConfig || agentDreamReady,
              ready: agentDreamReady,
              needsAction: agentDreamReady
                ? null
                : "Set CLAUDE_OS_AGENT_DREAM_DIR and schedule your agent's early-morning job (see README)",
              cost: "$0 extra — runs on your agent's existing plan",
            });
            ...
            // Container deployment: same honesty as Hermes above — detect a
            // containerized Claude Code home (env override or the conventional
            // /docker/<stack>/claude-home layout).
            const claudeContainerHome = (() => {
              if (claudeBin) return null;
              const envHome =
                process.env.CLAUDE_OS_CLAUDE_CODE_HOME ?? DOTENV.CLAUDE_OS_CLAUDE_CODE_HOME;
            ...
            engines.push({
              id: "claude",
              name: "Claude Code",
              description: claudeContainerHome
                ? "Claude Code detected (runs in a Docker container) — the dashboard can't execute it directly"
                : hasApiKey
              ...
              installed: !!claudeBin || !!claudeContainerHome,
              ready: !!claudeBin && claudeSkill && hasApiKey,
              needsAction: claudeContainerHome
                ? "Runs in a container — pick OpenRouter for Dream, or install the CLI on this machine"
                : !claudeBin
                  ? "Install Claude Code"
                  : !claudeSkill
          ...
          server.middlewares.use("/__trigger_dream", (req, res) => {
            ...
            // Agent-fulfilled engine has no local process to spawn — the
            // agent's own scheduled job produces the dream. Say so honestly
            // instead of pretending "Generate now" works for it.
            if (chosenEngine === "hermes-agent") {
              res.statusCode = 400;
              res.end(
                JSON.stringify({
                  ok: false,
                  error:
                    "Agent-fulfilled Dreams are generated by your agent's scheduled job — the next one lands automatically. To generate on demand, pick a local engine (e.g. OpenRouter).",
                }),
              );
              return;
            }
            ...
            async function runOpenRouterDream(apiKey: string): Promise<void> {
              ...
              const skillCandidates = [
                ...
                join(HERMES_HOME_DIR,"skills", "dream", "SKILL.md"),
    ...
    server: {
      ...
      host: "127.0.0.1",
      // Allow the deployment's reverse-proxy Host header(s) while keeping the
      // dev server bound to loopback only. Do not use allowedHosts: true.
      ...(TRUSTED_HOSTS.length > 0 ? { allowedHosts: TRUSTED_HOSTS } : {}),
      port: 8081,
      strictPort: true,
